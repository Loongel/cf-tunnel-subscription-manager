...
- Preferred Endpoint node-link recovery was applied on 2026-06-13: 2 exclusive endpoint scopes and 14 endpoint selections were restored from local D1 backups; no historical global exclusion rows were present. Root cause: the old admin UI always submitted `proxyNodeIds` / `excludedProxyNodeIds` on endpoint save, so saving while the node picker had no loaded selections caused the backend replacement logic to delete existing links and insert an empty set. The admin UI now preserves existing endpoint-node links for metadata-only saves, loads proxy nodes before editing node-linked endpoints, and blocks node-link saves if proxy nodes are not loaded.
- Worker deployed version `fcfed880-a9ed-4a96-96ab-52943af8e3d4`
