...
6. In `Preferred Endpoints`, paste IPs/domains or Discovery URLs separated by commas, spaces, or newlines into `Values`; one click should create multiple endpoint rows. Discovery endpoints read the final endpoint from an HTTP 301/302/303/307/308 `Location` or from a service information page during subscription generation. If `DISCOVERY_ACCESS_HEADER_VALUE` is configured, discovery requests include `x-woker-id`. `Global Always On` endpoints are automatically used by every tunnel-backed node. `Binding Option` endpoints are selected from `Traffic Binding`.
7. In `Proxy Nodes`, use `Import From Subscription` with either subscription URLs or pasted subscription content. Supported imports are share links including Hysteria2, base64 V2Ray-style subscriptions, and sing-box outbound JSON.
